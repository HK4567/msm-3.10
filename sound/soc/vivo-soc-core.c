	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(vivo_rtd, &runtime_list, list) {
		if (!strcmp(ucontrol->id.name, vivo_rtd->link->name)) {
			dev_err(vivo_rtd->codec->dev, "vivo-ASoC: %s get_volsw dai enable %d\n",
				ucontrol->id.name,
				vivo_rtd->enable);
			ucontrol->value.integer.value[0] = vivo_rtd->enable;
		}
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(vivo_rtd, &runtime_list, list) {
		if (!strcmp(ucontrol->id.name, vivo_rtd->link->name)) {
			dev_err(vivo_rtd->codec->dev, "vivo-ASoC: %s put_volsw dai enable %ld\n",
				ucontrol->id.name,
				ucontrol->value.integer.value[0]);
			vivo_rtd->enable = !!ucontrol->value.integer.value[0];
		}
	}
...
int init_vivo_soc_dais(struct snd_soc_card *card)
{
	...
	for (i = 0; i < card->num_links; i++) {
		...
		/* If cpu dai name doesn't include 'vivo-snd-soc-dummy-dai', ignore. */
		if (!(link->cpu_dai_name &&
			strstr(link->cpu_dai_name, "vivo-snd-soc-dummy-dai")))
			continue;
			      ...
			      int event)
{
	...
	dev_err(card->dev, "vivo-ASoC: vivo_snd_soc_dapm_stream_event stream %d\n", stream);
...
static int vivo_dummy_codec_set_sysclk_rx(struct snd_soc_dai *dai,
		int clk_id, unsigned int freq, int dir)
{
	...
	list_for_each_entry(vivo_rtd, &runtime_list, list) {
		if (vivo_rtd->enable &&
			!strcmp("vivo-snd-soc-dummy-dai-rx",
			vivo_rtd->link->cpu_dai_name)) {
			codec_dai = vivo_rtd->codec_dai;

			dev_err(codec_dai->dev, "vivo-ASoC: %s\n", __func__);
...
static int vivo_dummy_codec_set_pll_rx(struct snd_soc_dai *dai,
...
static int vivo_dummy_codec_set_clkdiv_rx(struct snd_soc_dai *dai,
		int div_id, int div)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_set_fmt_rx(struct snd_soc_dai *dai,
		unsigned int fmt)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_set_tdm_slot_rx(struct snd_soc_dai *dai,
...
static int vivo_dummy_codec_set_channel_map_rx(struct snd_soc_dai *dai,
...
static int vivo_dummy_codec_set_tristate_rx(struct snd_soc_dai *dai,
		int tristate)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_digital_mute_rx(struct snd_soc_dai *dai,
		int mute)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_mute_stream_rx(struct snd_soc_dai *dai,
		int mute, int stream)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
	int ret;
	// add conditions active mute and stream for bug[B160122-392] to fix issue that fm no sound output
	// from spk when double quick press switch in fm playing in spk mode, with touchsound is active. qbq.
	list_for_each_entry(vivo_rtd, &runtime_list, list) {
		if ((vivo_rtd->enable &&
			!strcmp("vivo-snd-soc-dummy-dai-rx",
			vivo_rtd->link->cpu_dai_name))
			|| (vivo_rtd->active && mute && !stream)) {
			codec_dai = vivo_rtd->codec_dai;

			dev_err(codec_dai->dev, "vivo-ASoC: %s  mute=%d.\n",__func__, mute);
...
static int vivo_dummy_codec_startup_rx(struct snd_pcm_substream *substream,
...
static void vivo_dummy_codec_shutdown_rx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_hw_params_rx(struct snd_pcm_substream *substream,
...
static int vivo_dummy_codec_hw_free_rx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_prepare_rx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_trigger_rx(struct snd_pcm_substream *substream,
...
static int vivo_dummy_codec_bespoke_trigger_rx(struct snd_pcm_substream *substream,
		int cmd, struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
/*
static snd_pcm_sframes_t vivo_dummy_codec_delay_rx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_set_sysclk_tx(struct snd_soc_dai *dai,
		int clk_id, unsigned int freq, int dir)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
	...
	list_for_each_entry(vivo_rtd, &runtime_list, list) {
		if (vivo_rtd->enable &&
			!strcmp("vivo-snd-soc-dummy-dai-tx",
			vivo_rtd->link->cpu_dai_name)) {
			codec_dai = vivo_rtd->codec_dai;
...
static int vivo_dummy_codec_set_pll_tx(struct snd_soc_dai *dai,
		int pll_id, int source,
		unsigned int freq_in,
		unsigned int freq_out)
{
...
static int vivo_dummy_codec_set_clkdiv_tx(struct snd_soc_dai *dai,
		int div_id, int div)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_set_fmt_tx(struct snd_soc_dai *dai,
		unsigned int fmt)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_set_tdm_slot_tx(struct snd_soc_dai *dai,
		unsigned int tx_mask, unsigned int rx_mask,
		int slots, int slot_width)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
...
static int vivo_dummy_codec_set_channel_map_tx(struct snd_soc_dai *dai,
		unsigned int tx_num, unsigned int *tx_slot,
		unsigned int rx_num, unsigned int *rx_slot)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
...
static int vivo_dummy_codec_set_tristate_tx(struct snd_soc_dai *dai,
		int tristate)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_digital_mute_tx(struct snd_soc_dai *dai,
		int mute)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_mute_stream_tx(struct snd_soc_dai *dai,
		int mute, int stream)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_startup_tx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static void vivo_dummy_codec_shutdown_tx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_hw_params_tx(struct snd_pcm_substream *substream,
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_hw_free_tx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_prepare_tx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_trigger_tx(struct snd_pcm_substream *substream,
		int cmd, struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
static int vivo_dummy_codec_bespoke_trigger_tx(struct snd_pcm_substream *substream,
		int cmd, struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
/*
static snd_pcm_sframes_t vivo_dummy_codec_delay_tx(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct vivo_snd_soc_runtime *vivo_rtd;
	struct snd_soc_dai *codec_dai;
...
*/
static struct snd_soc_dai_ops vivo_dummy_codec_rx_ops = {
	.set_sysclk = vivo_dummy_codec_set_sysclk_rx,
	.set_pll = vivo_dummy_codec_set_pll_rx,
	.set_clkdiv = vivo_dummy_codec_set_clkdiv_rx,

	.set_fmt = vivo_dummy_codec_set_fmt_rx,
	.set_tdm_slot = vivo_dummy_codec_set_tdm_slot_rx,
	.set_channel_map = vivo_dummy_codec_set_channel_map_rx,
	.set_tristate = vivo_dummy_codec_set_tristate_rx,

	.digital_mute = vivo_dummy_codec_digital_mute_rx,
	.mute_stream = vivo_dummy_codec_mute_stream_rx,

	.startup = vivo_dummy_codec_startup_rx,
	.shutdown = vivo_dummy_codec_shutdown_rx,
	.hw_params = vivo_dummy_codec_hw_params_rx,
	.hw_free = vivo_dummy_codec_hw_free_rx,
	.prepare = vivo_dummy_codec_prepare_rx,
	.trigger = vivo_dummy_codec_trigger_rx,
	.bespoke_trigger = vivo_dummy_codec_bespoke_trigger_rx,

	//.delay = vivo_dummy_codec_delay_rx,
};

static struct snd_soc_dai_ops vivo_dummy_codec_tx_ops = {
	.set_sysclk = vivo_dummy_codec_set_sysclk_tx,
	.set_pll = vivo_dummy_codec_set_pll_tx,
	.set_clkdiv = vivo_dummy_codec_set_clkdiv_tx,

	.set_fmt = vivo_dummy_codec_set_fmt_tx,
	.set_tdm_slot = vivo_dummy_codec_set_tdm_slot_tx,
	.set_channel_map = vivo_dummy_codec_set_channel_map_tx,
	.set_tristate = vivo_dummy_codec_set_tristate_tx,

	.digital_mute = vivo_dummy_codec_digital_mute_tx,
	.mute_stream = vivo_dummy_codec_mute_stream_tx,

	.startup = vivo_dummy_codec_startup_tx,
	.shutdown = vivo_dummy_codec_shutdown_tx,
	.hw_params = vivo_dummy_codec_hw_params_tx,
	.hw_free = vivo_dummy_codec_hw_free_tx,
	.prepare = vivo_dummy_codec_prepare_tx,
	.trigger = vivo_dummy_codec_trigger_tx,
	.bespoke_trigger = vivo_dummy_codec_bespoke_trigger_tx,

	//.delay = vivo_dummy_codec_delay_tx,
};
...
static struct snd_soc_dai_driver vivo_dummy_dai[] = {
	{
		.name = "vivo-snd-soc-dummy-dai-rx",
		.playback = {
		...
		.ops = &vivo_dummy_codec_rx_ops,
	},
	{
		.name = "vivo-snd-soc-dummy-dai-tx",
		.capture = {
			.stream_name	= "Capture",
			.channels_min	= 1,
			.channels_max	= 384,
...
static int vivo_snd_soc_dummy_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_register_codec(&pdev->dev, &vivo_dummy_codec, vivo_dummy_dai, 2);
	if (ret < 0)
		return ret;
