...
static int pn5xx_enable(struct pn5xx_dev *dev, int mode)
{
    ...
    pr_info("%s called clk_state:%d \n", __func__,dev->clk_state);
	if(dev->clk_state == false)
	{
    ...
    /* VIVO_NFC begin. */
    //gpio_set_value(dev->pvdden_gpio, 1);
    /* VIVO_NFC end. */
...
static void pn5xx_disable(struct pn5xx_dev *dev)
{
    ...
    /* power off */
    pr_info("%s power off clk_state:%d\n", __func__,dev->clk_state);
	if(dev->clk_state == true)
	{
    ...
    /* VIVO_NFC begin. */
    //gpio_set_value(dev->pvdden_gpio, 0);
    /* VIVO_NFC end. */
                ...
                unsigned long arg)
{
    ...
    case PN5XX_SET_PWR:
        if (arg == 2) {
            ...
            /* power off */
            pn5xx_disable(pn5xx_dev);
            disable_irq_wake(pn5xx_dev->client->irq);
            msleep(50);
        } else {
                           ...
                           struct pn5xx_i2c_platform_data *pdata)
{
    ...
    /* VIVO_NFC begin. */
	pdata->clkreq_gpio = -EINVAL;//zsy A
    val = of_get_named_gpio_flags(node, "nxp,pn5xx-pvdden_gpio", 0, &flags);
    if (val >= 0) {
        pdata->pvdden_gpio = val;
    }
    else {
        pdata->pvdden_gpio = -EINVAL;
        dev_warn(dev, "PVDD ENABLE GPIO <OPTIONAL> error getting from OF node\n");
    }
    /* VIVO_NFC end. */
...
*/
    /* VIVO_NFC begin. */
	//clk_prepare_enable(pn5xx_dev->clk);
    /* VIVO_NFC end. */
    return 0;
}

static int pn5xx_suspend(struct i2c_client *client, pm_message_t state)
{
...
#endif
{
    ...
    /* VIVO_NFC begin. */
    pdata->clk = clk_get(&client->dev, "ref_clk");
    if (IS_ERR(pdata->clk)) {
        dev_err(&client->dev, "Could not get ref_clk\n");
    ...
    /* VIVO_NFC begin. */
    if (gpio_is_valid(pdata->pvdden_gpio)) {
        pr_info("%s: request pvdden_gpio %d\n", __func__, pdata->pvdden_gpio);
        ret = gpio_request(pdata->pvdden_gpio, "nfc_pvdden");
        if (ret){
            pr_err("%s :not able to get GPIO pvdden_gpio\n", __func__);
            goto err_pvdden;
        }
    }
    /* VIVO_NFC end. */
...
*/
    ...
    /* VIVO_NFC begin. */
    pn5xx_dev->pvdden_gpio  = pdata->pvdden_gpio;
    /* VIVO_NFC end. */
    pn5xx_dev->p61_current_state = P61_STATE_IDLE;
    ...
    pn5xx_dev->clk = pdata->clk;

    pn5xx_dev->client   = client;
	pn5xx_dev->clk_state = true;
    ...
    /* VIVO_NFC begin. */
    pr_info("pn5xx is_poweroff_charge_mode:%d!\n", power_off_charging_mode);
    if (power_off_charging_mode == 0)
    {
        msleep(10);
        ret = gpio_direction_output(pn5xx_dev->pvdden_gpio, 1);
    }else{
        ret = gpio_direction_output(pn5xx_dev->pvdden_gpio, 0);
    }
    if (ret < 0) {
        pr_err("%s :not able to set pvdden_gpio as output\n", __func__);
    }
    /* VIVO_NFC end. */
/*
    if (gpio_is_valid(pn5xx_dev->clkreq_gpio)) {
...
#endif
{
    ...
    /* VIVO_NFC begin. */
    gpio_set_value(pn5xx_dev->pvdden_gpio, 0);
    /* VIVO_NFC end. */
    ...
    /* VIVO_NFC begin. */
    gpio_free(pn5xx_dev->pvdden_gpio);
    /* VIVO_NFC end. */
