...
#define AKM09912_VDD_MIN_UV	2000000
#define AKM09912_VDD_MAX_UV	3300000
#define AKM09912_VIO_MIN_UV	1750000
#define AKM09912_VIO_MAX_UV	1950000
...
static struct i2c_client *akm09912_mag_i2c_client=NULL;
	...
	int length)
{
	...
	if (ret < 0) {
		printk(KERN_ERR TAGI "%s: transfer failed.\n", __func__);
		return ret;
	} else if (ret != ARRAY_SIZE(msgs)) {
		printk(KERN_ERR TAGI "%s: transfer failed(size error).\n",
				__func__);
		return -ENXIO;
	}
	...
	int length)
{
	...
	if (ret < 0) {
		printk(KERN_ERR TAGI "%s: transfer failed.", __func__);
		return ret;
...
static int akm_compass_suspend(struct device *dev)
{
	...
	if (ret)
		printk(KERN_ERR TAGE "Failed to set to POWERDOWN mode.\n");
	...
	struct i2c_client *client)
{
	...
	if ((akm->sense_info[0] != AK09912_WIA1_VALUE) ||
		...
		printk(KERN_ERR TAGI
			"%s: The device is not AKM09912 Compass.", __func__);
		return -ENXIO;
	}
...
err_vio_disable:
	if (regulator_enable(data->vdd))
		printk(KERN_INFO TAGI "Regulator vdd enable failed\n");
err_vdd_disable:
	return rc;
}
	...
	if (!on) {
		...
		if (regulator_count_voltages(data->vdd) > 0) {
			rc = regulator_set_voltage(data->vdd,
				AKM09912_VDD_MIN_UV, AKM09912_VDD_MAX_UV);
			if (rc) {
				printk(KERN_ERR TAGE
		...
		if (regulator_count_voltages(data->vio) > 0) {
			rc = regulator_set_voltage(data->vio,
				AKM09912_VIO_MIN_UV, AKM09912_VIO_MAX_UV);
			if (rc) {
				printk(KERN_ERR TAGE
...
reg_vdd_set:
	if (regulator_count_voltages(data->vdd) > 0)
		regulator_set_voltage(data->vdd, 0, AKM09912_VDD_MAX_UV);
reg_vdd_put:
	regulator_put(data->vdd);
	return rc;
}
				...
				struct akm_compass_data *akm)
{
	...
	if (rc && (rc != -EINVAL)) {
		printk(KERN_ERR TAGI "Unable to read akm,layout\n");
		return rc;
	} else {
		akm->layout = temp_val;
	}
...
static void akm_dev_poll(struct work_struct *work)
{
	...
	if (ret < 0)
		printk(KERN_ERR TAGI "Failed to report data\n");
		...
		int *fail_total)
{
	...
	if (strcmp(test_name, "START") == 0) {
		printk(KERN_INFO TAGI "----------------------------------------------------------\n");
		printk(KERN_INFO TAGI "Test Name    Fail    Test Data    [      Low         High]\n");
		printk(KERN_INFO TAGI "----------------------------------------------------------\n");
	} else if (strcmp(test_name, "END") == 0) {
		printk(KERN_INFO TAGI "----------------------------------------------------------\n");
		if (*fail_total == 0)
			printk(KERN_INFO TAGI "Factory shipment test passed.\n\n");
		else
			printk(KERN_INFO TAGI "%d test cases failed.\n\n",
					*fail_total);
	} else {
		...
		printk(KERN_INFO TAGI " %-10s      %c    %9d    [%9d    %9d]\n",
...
static int akm_self_test(struct sensors_classdev *sensors_cdev)
{
    ...
    printk(KERN_INFO TAGI "%s %p %p\n", __func__, akm, s_akm);
	mutex_lock(&akm->op_mutex);
    printk(KERN_INFO TAGI "%s 1\n", __func__);
	asax = akm->sense_conf[AKM09912_AXIS_X];
	asay = akm->sense_conf[AKM09912_AXIS_Y];
	asaz = akm->sense_conf[AKM09912_AXIS_Z];
    printk(KERN_INFO TAGI "%s 2\n", __func__);
	if (!power_enabled) {
		ret = akm_compass_power_set(akm, true);
		if (ret) {
			printk(KERN_INFO TAGI "Power up failed.\n");
			goto exit;
		}
	} else {
		...
		if (ret < 0) {
			printk(KERN_INFO TAGI "Get mode failed.\n");
			goto exit;
		}
		mode = i2c_data[1];
	}
    printk(KERN_INFO TAGI "%s 3\n", __func__);
	ret = AKECS_Reset(akm, 0);
	if (ret < 0) {
		printk(KERN_INFO TAGI "Reset failed.\n");
		goto exit;
	}
    printk(KERN_INFO TAGI "%s 4\n", __func__);
	/* start test */
	case_test(akm, "START", 0, 0, 0, &fail_total);

	case_test(akm, TLIMIT_TN_ASAX_09912, asax, TLIMIT_LO_ASAX_09912,
			TLIMIT_HI_ASAX_09912, &fail_total);
	case_test(akm, TLIMIT_TN_ASAY_09912, asay, TLIMIT_LO_ASAY_09912,
			TLIMIT_HI_ASAY_09912, &fail_total);
	case_test(akm, TLIMIT_TN_ASAZ_09912, asaz, TLIMIT_LO_ASAZ_09912,
			TLIMIT_HI_ASAZ_09912, &fail_total);
    printk(KERN_INFO TAGI "%s 5\n", __func__);
	
	ret = AKECS_SetMode(akm, AK09912_MODE_SNG_MEASURE);
	if (ret < 0) {
		printk(KERN_INFO TAGI "Set to single measurement failed.\n");
		goto exit;
	}
	usleep(50000);
    printk(KERN_INFO TAGI "%s 6\n", __func__);
	count = AKM09912_RETRY_COUNT;
	do {
	...
	if (!count) {
		printk(KERN_INFO TAGI "Timeout get valid data.\n");
		goto exit;
	}
    printk(KERN_INFO TAGI "%s 7\n", __func__);
	hdata[AKM09912_AXIS_X] = (s16)(i2c_data[1] | (i2c_data[2] << 8));
	hdata[AKM09912_AXIS_Y] = (s16)(i2c_data[3] | (i2c_data[4] << 8));
	hdata[AKM09912_AXIS_Z] = (s16)(i2c_data[5] | (i2c_data[6] << 8));

	i2c_data[0] &= 0x7F;
	case_test(akm, TLIMIT_TN_SNG_ST1_09912,
	       (int)i2c_data[0], TLIMIT_LO_SNG_ST1_09912,
		TLIMIT_HI_SNG_ST1_09912, &fail_total);

	case_test(akm, TLIMIT_TN_SNG_HX_09912, hdata[0], TLIMIT_LO_SNG_HX_09912,
			TLIMIT_HI_SNG_HX_09912, &fail_total);
	case_test(akm, TLIMIT_TN_SNG_HY_09912, hdata[1], TLIMIT_LO_SNG_HY_09912,
			TLIMIT_HI_SNG_HY_09912, &fail_total);
	case_test(akm, TLIMIT_TN_SNG_HZ_09912, hdata[2], TLIMIT_LO_SNG_HZ_09912,
			TLIMIT_HI_SNG_HZ_09912, &fail_total);

	case_test(akm, TLIMIT_TN_SNG_ST2_09912, (int)i2c_data[8],
			TLIMIT_LO_SNG_ST2_09912, TLIMIT_HI_SNG_ST2_09912,
			&fail_total);
    printk(KERN_INFO TAGI "%s 8\n", __func__);

    #if 0
	ret = AKECS_SetMode(akm, AK09912_MODE_POWERDOWN);
	if (ret < 0) {
	...
	/* self-test mode */
	ret = AKECS_SetMode(akm, AK09912_MODE_SELF_TEST);
	if (ret < 0) {
		printk(KERN_INFO TAGI "Set to self test mode failed\n");
		goto exit;
	}
	usleep(50000);
    printk(KERN_INFO TAGI "%s 9\n", __func__);
	count = AKM09912_RETRY_COUNT;
	do {
		/* The typical time for single measurement is 7.2ms */
		ret = AKECS_GetData_Poll(akm, i2c_data, AKM_SENSOR_DATA_SIZE);
    ...
    printk(KERN_INFO TAGI "%s 10\n", __func__);
	i2c_data[0] &= 0x7F;

	case_test(akm, TLIMIT_TN_SLF_ST1_09912, (int)i2c_data[0],
			TLIMIT_LO_SLF_ST1_09912, TLIMIT_HI_SLF_ST1_09912,
			&fail_total);
	...
	case_test(akm, TLIMIT_TN_SLF_RVHX_09912, (hdata[0])*((asax - 128)/2/128 + 1),
			TLIMIT_LO_SLF_RVHX_09912, TLIMIT_HI_SLF_RVHX_09912,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_RVHY_09912, (hdata[1])*((asay - 128)/2/128 + 1),
			TLIMIT_LO_SLF_RVHY_09912, TLIMIT_HI_SLF_RVHY_09912,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_RVHZ_09912, (hdata[2])*((asaz - 128)/2/128 + 1),
			TLIMIT_LO_SLF_RVHZ_09912, TLIMIT_HI_SLF_RVHZ_09912,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_ST2_09912, (int)i2c_data[8],
			TLIMIT_LO_SLF_ST2_09912, TLIMIT_HI_SLF_ST2_09912,
			&fail_total);
	...
	if (!power_enabled) {
		ret = akm_compass_power_set(akm, false);
		if (ret) {
			printk(KERN_INFO TAGI "Power down failed.\n");
			goto exit;
		}
	} else {
		/* Set measure mode */
		...
		if (ret < 0) {
			printk(KERN_INFO TAGI "restore mode failed\n");
			goto exit;
		}
	}
...
static int akm_read_data(struct akm_compass_data *akm)
{
	...
	if (STATUS_ERROR(dat_buf[8])) {
		printk(KERN_INFO TAGI "Status error. Reset...\n");
		AKECS_Reset(akm, 0);
		return -EIO;
	}
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	if(akm09912_mag_i2c_client)akm=i2c_get_clientdata(akm09912_mag_i2c_client);
...
static ssize_t akm09912_mag_shipment_test(struct device_driver *ddri, char *buf)
{
    int error=0;
...
static ssize_t show_akmd09912_layout(struct device_driver *ddri, char *buf)
{
	int layout_value = -1;
	struct akm_compass_data *akm = NULL;
	
	if(akm09912_mag_i2c_client)
	{
		akm = i2c_get_clientdata(akm09912_mag_i2c_client);
	}
...
static ssize_t store_akmd09912_layout(struct device_driver *ddri, const char *buf, size_t count)
{
    int layout_value = -1;
	struct akm_compass_data *akm = NULL;
...
static DRIVER_ATTR(selftest,S_IRUGO | S_IWUSR, akm09912_mag_shipment_test, NULL);
static DRIVER_ATTR(data,    S_IRUGO, show_sensordata_value, NULL);
static DRIVER_ATTR(layout,  S_IRUGO | S_IWUSR, show_akmd09912_layout, store_akmd09912_layout);

static struct driver_attribute *akm09912_mag_attr_list[] = {
	&driver_attr_selftest,
	&driver_attr_data,
	&driver_attr_layout,
};

static int akm09912_mag_create_attr(struct device_driver *driver) 
{
	int i;
	for (i = 0; i < ARRAY_SIZE(akm09912_mag_attr_list); i++)
		if (driver_create_file(driver, akm09912_mag_attr_list[i]))
			goto error;
	return 0;

error:
	for (i--; i >= 0; i--)
		driver_remove_file(driver, akm09912_mag_attr_list[i]);
	printk(KERN_ERR TAGE "%s:Unable to create interface\n", __func__);
	return -1;
}

static int akm09912_mag_pd_probe(struct platform_device *pdev) 
{
	return 0;
}

static int akm09912_mag_pd_remove(struct platform_device *pdev)
{
	return 0;
}

static struct platform_driver akm09912_mag_pd_driver = {
	.probe  = akm09912_mag_pd_probe,
	.remove = akm09912_mag_pd_remove,    
	.driver = {
		.name  = "msensor",
		.owner = THIS_MODULE,
	}
...
struct platform_device akm09912_mag_pd_device = {
    .name = "msensor",
    .id   = -1,
};

int akm09912_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		/* Platform data is not available.
		   Layout and information should be set by each application. */
			s_akm->layout = 0;
			s_akm->gpio_rstn = 0;
			printk(KERN_ERR TAGI "%s: No platform data.",
				__func__);
		}
	}
	
	if(0<=s_akm->iic_add && 0xff>=s_akm->iic_add)
	{
		printk(KERN_INFO TAGI "%s change the iic address from %x to %x\n",
		__func__, client->addr, s_akm->iic_add);
		client->addr=s_akm->iic_add;
	}
...
*/
    ...
    if((err = platform_driver_register(&akm09912_mag_pd_driver)))
	{
		printk(KERN_ERR TAGE "%s failed to register akm09912_mag_driver err %d\n", __func__, err);
		goto err_unreg_sensor_class;
	}

    if((err = platform_device_register(&akm09912_mag_pd_device)))
    {
		printk(KERN_ERR TAGE "%s failed to register akm09912_mag_device err %d\n", __func__, err);
		goto err_free_driver;
    }

	if((err = akm09912_mag_create_attr(&akm09912_mag_pd_driver.driver)))
	{
		printk(KERN_ERR TAGE "%s akm09912 create attribute err = %d\n", __func__, err);
		goto err_free_device;
	}
    akm09912_mag_i2c_client=client;

	printk(KERN_ERR TAGI "akm09912 successfully probed.\n");
	return 0;

err_free_device:
	platform_device_unregister(&akm09912_mag_pd_device);
err_free_driver:
	platform_driver_unregister(&akm09912_mag_pd_driver);
...
exit1:
exit0:
	printk(KERN_ERR TAGI "akm09912 probed error.\n");
	return err;
}
...
static int akm_compass_remove(struct i2c_client *client)
{
	...
	if (akm_compass_power_set(akm, 0))
		printk(KERN_ERR TAGE "power set failed.");
	...
	if (misc_deregister(&akm_compass_dev) < 0)
		printk(KERN_ERR TAGE "misc deregister failed.");
	if (akm->irq)
		free_irq(akm->irq, akm);
	input_unregister_device(akm->input);
	kfree(akm);
	printk(KERN_INFO TAGI "akm09912 successfully removed.");
	return 0;
}
...
static struct of_device_id akm09912_match_table[] = {
...
static struct i2c_driver akm_compass_driver = {
	.probe		= akm09912_compass_probe,
	.remove		= akm_compass_remove,
	.id_table	= akm_compass_id,
	.driver = {
		.name	= AKM_I2C_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = akm09912_match_table,
...
{
	printk(KERN_INFO TAGI "AKM compass driver: initialize.");
