...
//	struct apds9922_priv *obj = i2c_get_clientdata(client);	
	...
	buffer[0]=APDS9922_DD_MAIN_CTRL_ADDR;
	res = apds9922_read_reg(client, buffer[0], &enable, 0x1);
	
    buffer[0]=APDS9922_DD_PRX_LED_ADDR;
	res = apds9922_read_reg(client, buffer[0], buffer, 0x1);
	if (buffer[0] == 0x36 || enable == 0x00 ) {
		printk(KERN_INFO TAGI "apds9922_read_ps init reg!\r\n");
		apds9922_enable_ps(client,0);
		mdelay(1);
		apds9922_enable_ps(client, 1);
	}
...
static int apds9922_get_ps_value(struct apds9922_priv *obj, u16 ps)   //xuling: this function only used in intterupt now
{
	int val;
...
static void apds9922_eint_work(struct work_struct *work)   //xuling: must have a mutex lock between intterrupt and data getting
{
...
static long apds9922_enable_als(struct i2c_client *client, int enable)
{
    ...
    if (enable) {
        //No need to write the MAIN_CTRL again when als&ps have already been enabled
        if(databuf[0] != 0x03)
        {
    ...
    {
        //Donot disable the als as the ps will disable both when ps disabled
        //since write the main ctrl register will break the ps measurement
        if (!test_bit(CMC_BIT_PS, &obj->enable) && !test_bit(CMC_BIT_ENG_PS, &obj->enable))
        {
            databuf[1] = databuf[0]& ~APDS9922_DD_ALS_EN;
...
MODULE_AUTHOR("xuguodong <xuling@vivo.com.cn>");
MODULE_LICENSE("GPL");
