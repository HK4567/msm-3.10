...
#define AKM_HAS_RESET			1 /*laizhilong modified 0*/
...
*/
	...
	//xuling:add for check if sensor on after 
...
static struct sensors_classdev sensors_cdev = {
	...
	.enabled = 0,
	.delay_msec = 200/*laizhilong modified 10000*/,
	.sensors_enable = NULL,
	...
	int length)
{
	...
	if (ret < 0) {
		printk(KERN_INFO TAGI "%s: transfer failed.\n", __func__);
		return ret;
	} else if (ret != ARRAY_SIZE(msgs)) {
		printk(KERN_INFO TAGI "%s: transfer failed(size error).\n",
				__func__);
		return -ENXIO;
	}
    if(DEBUG)
	...
	int hard)
{
	...
	if (hard != 0) {
		/*laizhilong remove gpio_set_value(akm->gpio_rstn, 0);*/
		udelay(5);
		/*laizhilong remove gpio_set_value(akm->gpio_rstn, 1);*/
		...
		unsigned int enable)
{
	...
	printk(KERN_INFO TAGI "%s enable %d is_busy %d\n", __func__, enable, akm->is_busy);/*laizhilong add*/
		...
		unsigned int delay_msec)
{
	struct akm_compass_data *akm = container_of(sensors_cdev,
			struct akm_compass_data, cdev);
    printk(KERN_INFO TAGI "%s delay_msec %u\n", __func__, delay_msec);/*laizhilong add debug*/
	mutex_lock(&akm->val_mutex);
	...
	struct akm_compass_data *akm, char const *buf, size_t count, int pos)
{
	...
	if (kstrtoul(buf, AKM_BASE_NUM, &val))
		return -EINVAL;
	printk(KERN_INFO TAGI "%s sensor %d delay_msec %lu\n", __func__, pos, val);/*laizhilong add debug*/
...
static irqreturn_t akm_compass_irq(int irq, void *handle)
{
	...
	uint8_t buffer[AKM_SENSOR_DATA_SIZE];
	int err;
	//xuling: add for check continuous interrupts
	...
	struct i2c_client *client)
{
	...
	if (akm->sense_info[0] != AK8963_WIA_VALUE) {
		printk(KERN_INFO TAGI
			"%s: The device is not AKM8963 Compass.\n", __func__);
		return -ENXIO;
	}
...
static int akm_compass_power_set(struct akm_compass_data *data, bool on)
{
	...
	if (!on && data->power_enabled) {
		/*laizhilong remove for crash start*/
		...
		/*laizhilong remove for crash end*/
				...
				struct akm8963_platform_data *pdata)
{
	...
	if (rc && (rc != -EINVAL)) {
		printk(KERN_INFO TAGI "Unable to read akm,layout\n");
		return rc;
	} else {
    ...
    /*laizhilong modified reset pin connect to vddio start*/
          ...
          int * pf_total)
{
	...
	if ((testno == NULL) && (strncmp(testname, "START", 5) == 0)) {
		...
		if (*pf_total == 1) {
			...
			printk(KERN_INFO TAGI "Factory shipment test was failed.\n\n");
		}
...
int FST_AK8963(void)
{
	...
	if(CSPEC_SPI_USE == 1){
		...
		if (AKI2C_TxData(i2cData, 2) < 0) {
			printk(KERN_INFO TAGI "%s:%d Error.\n", __FUNCTION__, __LINE__);
			goto error_exit;
		}
	}
...
static ssize_t akm8963_mag_shipment_test(struct device_driver *ddri, char *buf)
{
	...
	{
	   printk(KERN_INFO TAGI "shipment_test fail\n");
	   //strcpy(result,"n");
	   ret = -1;
	}
	else
...
//	ktime_t timestamp;
	...
	//xuling: add for check poll delay long
...
//	input_sync(akm->input);
    ...
    /*laizhilong add start*/
	mutex_lock(&akm->sensor_data_mutex);
	 //xuling: add for same data detect
...
//xuling:add for check if sensor on after screen off
#if defined(CONFIG_FB)
		...
		const struct i2c_device_id *id)
{
	...
	if (i2c->dev.of_node) {
		...
		if (i2c->dev.platform_data) {
			...
			s_akm->gpio_rstn = 0;
			printk(KERN_INFO TAGI "%s: No platform data.\n",
					__func__);
		}
	}
...
*/
	printk(KERN_INFO TAGI "akm8963 successfully probed.\n");
	return 0;
/*laizhilong add start*/
err_free_device:
...
err_i2c_check:
	printk(KERN_INFO TAGI "akm8963 probed error.\n");
	return err;
}
...
static int akm8963_compass_remove(struct i2c_client *i2c)
{
	...
	printk(KERN_INFO TAGI "akm8963 successfully removed.\n");
	return 0;
}
...
{
	printk(KERN_INFO TAGI "akm8963 compass driver: release.\n");
