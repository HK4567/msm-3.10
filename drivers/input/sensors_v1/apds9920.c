...
//vivo haosirong add for apds9930 ams para
...
//	int APDS9920_OUTDOOR_GA = 250; //125;
	// printk("xuguodong als c0 = %d c1 = %d\r\n",ch0data,ch1data);
	...
	if ( (ch1ch0_ratio >= 30) && (ch1ch0_ratio <= 45) ) {
		...
		luxValue = (luxValue * APDS9920_OUTDOOR_GA)/100;
	         printk("xuguodong als  11 luxValue = %d\r\n ",luxValue);
...
//	}
	...
	//printk("xuguodong IAC1 = %d IAC2 = %d\r\n",IAC1,IAC2);
	if (IAC1 > IAC2) {
		IAC = IAC1;
	} else if (IAC1 <= IAC2) {
		IAC = IAC2;
...
//	int dark_light = 0;
	...
	if(!is_short_atime_mode){
		if(c0_value >= (apds9920_als_res_tb[obj->als_atime_index] * 85 / 100) && obj->als_last_atime_index == 1) {
			...
			//printk("xuguodong atime value = %d\r\n",buffer[0]);
		}
	}
#endif
	if(change > 0)
...
/*-----------------------------------------------------------------------------*/
static int apds9920_get_ps_value(struct apds9920_priv *obj, u16 ps)   //xuling: this function only used in intterupt now
{
	int val;
...
//lianghuanhuan: add for check continuous interrupts
	static int cont_int_num = 0;
    	static unsigned long last_int_time = 0;
    	unsigned long int_time;
    	unsigned long diff;
//lianghuanhuan add end;    
...
/*----------------------------------------------------------------------------*/
static void apds9920_eint_work(struct work_struct *work)   //xuling: must have a mutex lock between intterrupt and data getting 
{
...
//	printk(KERN_INFO TAGI "apds9920_eint_work\n");
	...
	apds9920_clear_intr(obj->client);//lianghuanhuan add 2015.5.21;
    ...
    enable_irq(obj->irq);    //xuling: irq will enable in ps_report_work
...
static ssize_t store_ps_base_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	//unsigned char databuf[2];
	//int res;
//	printk(KERN_INFO TAGI "lianghuanhuan add------0\n");
...
static void apds9920_ps_data_report(struct work_struct *work)
{
    ...
    u16 als_c0_dat;
    int err = 0;
//printk(KERN_INFO TAGI "lianghuanhuan add------1\n");	
...
restart_poll:
//	if(debug)
//	printk(KERN_INFO TAGI "liang add &data->als_suspend = %d\n", atomic_read(&data->als_suspend));//lianghuanhuan add
...
//	atomic_set(&obj->als_suspend, 0);
    ...
    INIT_WORK(&obj->eint_work, apds9920_eint_work);        //xuling: now we used interrupt mode
