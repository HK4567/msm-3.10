...
static int apds9960_ps_select_para(struct apds9960_data *obj)         //
{
    ...
    //TODO: xuling we do not have parameters now
...
static int apds9960_get_ps_value(struct apds9960_data *obj, unsigned int ps)   //xuling: this function only used in intterupt now
{
    int val;
...
static int LuxCalculation(struct i2c_client *client)
{
    ...
    {	
        n = 200 - data->cdata;
        printk("xuguodong-als befor  cdata = %d n = %d\r\n",cdata,n);
        cdata = cdata * powf(1.011, n);
        printk("xuguodong-als after  cdata = %d\r\n", cdata);
    }*/
...
static int apds9960_read_ps(struct i2c_client *client, unsigned char *data)
{
	...
	if((mode & 0x01) == 0x01 && apds9960_obj->enable_gesture_sensor == APDS_ENABLE_GESTURE)
	{
        ...
        return -1;
        //xuling: fifo data can only be read in interrupt.
...
static int FilterGestureRawData(gesture_data_type *gesture_in_data, gesture_data_type *gesture_out_data)
{
    ...
    printk("[APDS9960]xuling u_delta=%d, d_delta=%d\r\n", u_delta, d_delta);
    printk("[APDS9960]xuling l_delta=%d, r_delta=%d\r\n", l_delta, r_delta);
...
static int DecodeGesture(int gesture_mode)
{
	...
	printk("[APDS9960] xuling ud_count=%d, lr_count=%d\r\n", gesture_ud_count, gesture_lr_count);
    printk("[APDS9960] xuling gst_ud_delta=%d, gst_lr_delta=%d\r\n", gesture_ud_delta, gesture_lr_delta);
...
static int GestureDataProcessing()
{
    ...
    printk("[APDS9960]xuling ud_delta=%d, lr_delta=%d\r\n", ud_delta, lr_delta);
...
process: 
    ...
    if(gesture_delta_count == MAX_DELTA_BUFFER_NUM)
    {
        gesture_ud_delta = 0;
        gesture_lr_delta = 0;
        printk(KERN_ERR"[APDS9960] xuling gesture time too long\r\n");
    }
    else
    {
        gesture_ud_delta = 0;
                ...
                if(GestureDataProcessing() > 0) 
                {
                    ...
                    if((gctrl & 0x01) == 0x01)        //Gmod is on
				    {
					    ...
					    { 
					        ...
					        cancel_delayed_work(&data->gs_resume_work);    //xuling: note already have lock
...
static int apds9960_init_client(struct i2c_client *client)
{
    ...
    //xuling:we do not use als interruput now
