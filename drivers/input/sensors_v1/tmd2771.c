...
#include <linux/bbk_drivers_info.h>    /* wangyuanliang add */
#endif
...
struct tmd2771_priv {
    ...
    //xuling: for interrup work mode support
...
static void tmd2771_enable_irq(struct tmd2771_priv *obj, int enable);
/******************************************************************************
 * local Variable
*******************************************************************************/
...
static u8 irq_count = 1;//default irq is enable
...
int tmd27723_read_als0(struct i2c_client *client, u16 *data)
{
	...
	else if(obj->als_mode == ALS_NORMAL_LUX_MODE && c0_value <= ALS_LUX_LOW_THRESHOLD)
	{
		...
		obj->als_modulus = (400*100)/(16*DEFAULT_ATIME);
		*data = obj->last_als_data;
		obj->strong_sunlight=DEFAULT_STRONG_SUNLIGHT*12/10;
		return 0;
	}
    //vivo xuling add for ams lux 
...
/*-----------------------------------------------------------------------------*/
static int tmd2771_get_ps_value(struct tmd2771_priv *obj, u16 ps)   //xuling: this function only used in intterupt now
{
	int val;
...
static irqreturn_t tmd2771_irq(int irq, void *data)
{
   ...
   //haosirong add for check continuous interrupts
	static int cont_int_num = 0;
    static unsigned long last_int_time = 0;
    unsigned long int_time;
    unsigned long diff;
	//haosirong add end;     
    tmd2771_enable_irq(obj,0);
...
/*----------------------------------------------------------------------------*/
static void tmd2771_eint_work(struct work_struct *work)   //xuling: must have a mutex lock between intterrupt and data getting 
{
	struct tmd2771_priv *obj = (struct tmd2771_priv *)container_of(work, struct tmd2771_priv, eint_work);
    //hwm_sensor_data sensor_data;
	printk(KERN_INFO TAGI"tmd2771_eint_work+++, CMC_BIT_ENG_PS = %d\n", test_bit(CMC_BIT_ENG_PS,&obj->enable));
...
//    printk(KERN_INFO TAGI"tmd2771_eint_work\n");
	...
	tmd2771_clear_intr(obj->client);//lianghuanhuan add 2015.5.21;
    if(atomic_read(&obj->ps_suspend))
        msleep(10);     //10ms 
    
    //vivo xuling add for report data immediately
    tmd2771_ps_data_report(&(obj->ps_report_work.work));

    tmd2771_enable_irq(obj, 1);    //xuling: irq will enable in ps_report_work
...
int tmd2771_setup_eint(struct i2c_client *client)
{
    ...
    if(!obj->irq)           //xuling:only init onece
    {
...
int tmd2771_enable_ps(struct i2c_client *client, int enable)
{
	...
	if(enable)
	{
	    ...
	    if(0 == obj->hw->polling_mode_ps)
		{
			...
			databuf[1] = 0x11;                        //xuling: ps eint debouce 1 cycle, als eint debounce 1 cycle.
			res = tmd2771_write_reg(client, databuf[0], &databuf[1], 0x1);
			if(res <= 0)
				goto EXIT_ERR;
	...
	else
	{
        ...
        if(0 == obj->hw->polling_mode_ps)
		{
            //cancel_work_sync(&obj->eint_work);          //xuling: can't open the sync
	...
	switch (cmd)
	{
		case ALSPS_SET_PS_MODE:
			...
			else
			{
				...
				clear_bit(CMC_BIT_ENG_PS, &obj->enable);//lianghuanhuan add to clear BIT_ENG_PS first
				if(!test_bit(CMC_BIT_PS,&obj->enable) && !test_bit(CMC_BIT_PS_ACROSS,&obj->enable)){
...
//				clear_bit(CMC_BIT_ENG_PS, &obj->enable);
	...
	/*end liujie 2011-11-16 modify*/			
		...
		case ALSPS_SET_ALS_MODE:
			...
			if(enable)
			{
				printk(KERN_INFO TAGI "%s-----enable als\n",__func__);// lianghuanhuan add
			...
			{
				printk(KERN_INFO TAGI "%s-----disable als\n",__func__);// lianghuanhuan add
				clear_bit(CMC_BIT_ENG_ALS, &obj->enable);//lianghuanhuan add to clear CMC_BIT_ENG_ALS first
...
static void tmd2771_enable_irq(struct tmd2771_priv *obj, int enable)
{
	if (enable == 1) 
	{
		...
		{
			enable_irq(obj->irq);
			printk(KERN_INFO TAGI "[%s]:set irq_count to 1 \n", __func__);
		}
		if(irq_count >= 255)
			irq_count = 1;
		printk(KERN_INFO TAGI "[%s]:enable_irq, irq_count is [%d] \n", __func__, irq_count);
	} 
	else
	{
		/* disable */
		...
		irq_count = 0;
		printk(KERN_INFO TAGI "[%s]:set irq_count to 0 \n", __func__);
	}
}
...
static int tmd2771_i2c_suspend(struct i2c_client *client, pm_message_t msg) 
{
    ...
    //xuling: for interrupt mode do noting, let the up layer close the sensor 
		...
		unsigned int enable)
{
    ...
    {
     	printk(KERN_INFO TAGI" %s disable ps_across\n", __func__);
...
static int tmd2771_psensor_setup(struct tmd2771_priv *obj)
{
    ...
    //xuling: modify change the ps input type to EV_REL
...
static void tmd2771_ps_across_data_report(struct work_struct *work)
{
	...
	if(debug)
	{
		printk(KERN_INFO TAGI" %s report across data\n", __func__);
	}
	...
	if(!test_bit(CMC_BIT_PS_ACROSS, &data->enable))
	{
		printk(KERN_INFO TAGI" %s cancel ps across delay_work", __func__);
		cancel_delayed_work(&data->ps_across_report_work);	
	}
	else
	{
...
static int tmd2771_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//nPluseCount = 4; 
	//haosirong:adapter paras from vivo sensor config
...
//			printk(KERN_INFO TAGI "[%s]:the glass para has been configured \n", __func__ );
		...
		else if(strstr(bbk_product_model_for_alsps,"PD1510"))
		{
		    DGF = 336000L;
			BCOEF = 1860L;
			CCOEF =750L;
    ...
    //xuling:for interrupt mode
    obj->eint_queue = create_singlethread_workqueue("tmd2771_eint");
    INIT_WORK(&obj->eint_work, tmd2771_eint_work);        //xuling: now we used interrupt mode
...
//	ps_data_same_count=0;
	...
	/* wangyuanliang add for devices list */
#if defined(CONFIG_BBK_DEVICE_INFO)
...
static int __init tmd2771_init(void)
{
    ...
    //xuling: init the wakr lock here
    wake_lock_init(&ps_suspend_lock, WAKE_LOCK_SUSPEND, "PS wakelock");
