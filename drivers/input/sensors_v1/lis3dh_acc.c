...
/* end of flyshine */
#define PHONE_MOVING_THRESHOLD 25
...
static int lis3dh_acc_hw_init(struct lis3dh_acc_data *acc)
{
	...
	if (err < 0) {
		printk(KERN_ERR TAGI "Error reading WHO_AM_I: is device available/working?\n");
		goto err_firstread;
	} else
		acc->hw_working = 1;
...
err_resume_state:
	...
	printk(KERN_ERR TAGI "lis3dh hw init error 0x%x,0x%x: %d\n", buf[0],buf[1], err);
	return err;
}
...
int lis3dh_acc_update_g_range(struct lis3dh_acc_data *acc, u8 new_g_range)
{
	...
	return err;
error:
	printk(KERN_ERR TAGI "update g range failed 0x%x,0x%x: %d\n",buf[0], buf[1], err);
			...
			struct lis3dh_acc_platform_data *pdata)
{
	...
	*/
	...
	{
        pdata->gpio_int1= -2;
        printk(KERN_ERR TAGI "%s no sensor,gpio-irq\n", __func__);
	}
	/*rc = of_get_named_gpio_flags(dev->of_node,
				"st,gpio-int2", 0, NULL);
	if (rc < 0) {
		...
		const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		printk(KERN_ERR TAGI "update_g_range failed\n");
		goto  err_power_off;
	}
...
exit_check_functionality_failed:
	printk(KERN_ERR TAGI "%s: Driver Init failed\n", LIS3DH_ACC_DEV_NAME);
	return err;
}
...
static int lis3dh_acc_resume(struct i2c_client *client)
{
	...
	//weitianlei:add for check if sensor on after screen off
    if(atomic_read(&acc->enabled)) 
        printk(KERN_ERR TAGI "sensor on after suspend\n");
    //weitianlei:add end
