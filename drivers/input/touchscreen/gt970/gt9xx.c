...
static int sleep_flag = 0; //zhj add for suspend mode and edge_suppress_switch
...
static void goodix_dclick_timer_work_func(struct work_struct *work)
{
	struct goodix_ts_data *sensor = container_of(work, struct goodix_ts_data ,
													dclick_timer_work);
	if(!sensor){
		VIVO_TS_LOG_ERR("[GTP]%s:Fail to get info data.\n",__func__);
		return ;
	}
...
static void goodix_dclick_timer_func(unsigned long data)
{
	...
	VIVO_TS_LOG_INF("[GTP] %s: dclick timer time out\n", __func__);
	VIVO_TS_LOG_INF("[GTP] %s: is this time dclick valide -- %s\n",
				__func__, sensor->is_dclick_valide?"yes":"no");
	VIVO_TS_LOG_INF("[GTP] %s: pressed_count(%d) release_count(%d)\n",
				__func__, sensor->pressed_count, sensor->release_count);
	VIVO_TS_LOG_INF("[GTP] %s: first_press_x(%d) first_press_y(%d)\n",
				__func__, sensor->first_press_x, sensor->first_press_y);
	VIVO_TS_LOG_INF("[GTP] %s: second_press_x(%d) second_press_y(%d)\n",
...
static void goodix_judge_dclick(struct goodix_ts_data *sensor,int x,int y)
{
	...
	VIVO_TS_LOG_INF("[GTP] %s: enter\n", __func__);

	if (sensor->num_fingers > 1) {
		VIVO_TS_LOG_INF("%s: More than one finger pressed on the TP\n", __func__);
	...
	if (!pre_state && !finger_0_state) {
		...
		VIVO_TS_LOG_INF("[GTP] %s: nothing to do.\n", __func__);
		...
		if (!whether_point_legal) {
			if (sensor->has_dclick_timer_start) {
				VIVO_TS_LOG_INF("%s: The point not in dclick dimension cancel trace\n",
						__func__);
				goodix_cancel_dclick_trace(sensor);
			}else{
				VIVO_TS_LOG_INF("%s: The point not in dclick dimension nothing to do\n",
						__func__);
			}
		...
		if (!sensor->has_dclick_timer_start) {
			...
			VIVO_TS_LOG_INF("[GTP] %s: first press start timer\n", __func__);
			...
			VIVO_TS_LOG_INF("[GTP] %s: second press start x(%d), y(%d)\n", __func__, x, y);
			...
			if ((x - sensor->first_press_x < -DCLICK_TWO_FINGER_AREA_X
					...
					|| (y - sensor->first_press_y < -DCLICK_TWO_FINGER_AREA_Y
						|| y - sensor->first_press_y > DCLICK_TWO_FINGER_AREA_Y)) {
				VIVO_TS_LOG_INF("[GTP] %s: The distance of the two down is too large\n", 
								__func__);
				goodix_cancel_dclick_trace(sensor);
			}
		}
			...
			if (sensor->pressed_count == 1 && sensor->release_count == 0) {
				if ((x - sensor->first_press_x < -TRIP_X_AREA 
						...
						|| (y - sensor->first_press_y < -TRIP_Y_AREA
							|| y - sensor->first_press_y > TRIP_Y_AREA)) {
					VIVO_TS_LOG_INF("[GTP] %s: finger triped in one time down\n", __func__);
					goodix_cancel_dclick_trace(sensor);
				}
 ...
 *
 * note:
 * 1.gt9286 must insert in 3 place,and we can serch by "vivo_ts_touch_info_print" in this driver
 */
void vivo_ts_touch_info_print(int touch_id, int x, int y, int is_down) {
	struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client);
	if(touch_id<0 || touch_id>=10) {
		return;
	}

	if(1 == is_down) {
		ts->finger_press_record[touch_id][0] = x;		//x
		ts->finger_press_record[touch_id][1] = y;		//y
		//just print log while first report this finger
		ts->finger_press_record[touch_id][2]++;		//report rate=80,so 80*60*60=288000timers/hour
		if(ts->finger_press_record[touch_id][2] == 1){
			VIVO_TS_LOG_INF("[%s]TouchInfo[%d][%d][%d][%d]\n", __func__, touch_id, 1,
				ts->finger_press_record[touch_id][0], ts->finger_press_record[touch_id][1]);
		}
	}
	if(0 == is_down) {
		ts->finger_press_record[touch_id][2] = 0;		//clear count,report rate=80,so 80*60*60=288000timers/hour
		VIVO_TS_LOG_INF("[%s]TouchInfo[%d][%d][%d][%d]\n", __func__, touch_id, 0,
				ts->finger_press_record[touch_id][0], ts->finger_press_record[touch_id][1]);
	}
}
...
if(vivo_touchscreen_is_support(FS_LARGE_OBJ_SUPPRESSION)) {
	...
	{	
		...
		{
			ts->largetouch_flag = 0;
			ts->is_calling = ts->is_calling_save;
		}
	}
...
#endif
					vivo_ts_touch_info_print(id, input_x, input_y, 1);
	//[PLATFORM] 0==strcmp(global_product_name,"PD1505") ||0==strcmp(global_product_name,"PD1523A") ||0==strcmp(global_product_name,"PD1524")
					if(0==strcmp(global_product_name,"PD1523A") && pre_touch && ((input_x < 80 && input_y > 1930) || (input_x > 990 && input_y > 1930)))
					{
						gtp_touch_up(ts, i);
	                	pre_touch &= ~(0x01 << i);
						VIVO_TS_LOG_INF("==PD1523A 2D finger down== id:%d, input_x:%d, input_y:%d, input_w:%d\n", id, input_x, input_y, input_w);
					}
					else if(0==strcmp(global_product_name,"PD1524") && pre_touch && ((input_x < 50 && input_y > 1290) || (input_x > 680 && input_y > 1290)))
					{
						gtp_touch_up(ts, i);
	                	pre_touch &= ~(0x01 << i);
						VIVO_TS_LOG_INF("==PD1524 2D finger down== id:%d, input_x:%d, input_y:%d, input_w:%d\n", id, input_x, input_y, input_w);
					}
					else if(0==strcmp(global_product_name,"PD1505") && pre_touch && ((input_x < 20 && input_y > 980) || (input_x > 520 && input_y > 980)))
					{
						gtp_touch_up(ts, i);
	                	pre_touch &= ~(0x01 << i);
						VIVO_TS_LOG_INF("==PD1505 2D finger down== id:%d, input_x:%d, input_y:%d, input_w:%d\n", id, input_x, input_y, input_w);
					}
					else if(0==strcmp(global_product_name,"PD1510") && pre_touch && ((input_x < 20 && input_y > 980) || (input_x > 520 && input_y > 980)))
					{
						gtp_touch_up(ts, i);
	                	pre_touch &= ~(0x01 << i);
						VIVO_TS_LOG_INF("==PD1510 2D finger down== id:%d, input_x:%d, input_y:%d, input_w:%d\n", id, input_x, input_y, input_w);
					}
					else
					{
	            ...
	            {            	
					if (pre_touch & (0x01 << i))
					{
						vivo_ts_touch_info_print(i, 0, 0, 0);
					}
	                gtp_touch_up(ts, i);
	                pre_touch &= ~(0x01 << i);
	            }
...
static int gtp_check_product_id(struct i2c_client *client)
{
	...
	VIVO_TS_LOG_INF("Goodix Product ID = %s\n", product_id);
					...
					struct kobj_attribute *attr,  const char *buf, size_t count)
{
	...
	if (val == 0) {
		ts->is_calling = 0;
		ts->is_calling_save = 0;
	}else if (val == 1 && ts->largetouch_flag == 0) {
		ts->is_calling = 1;
		ts->is_calling_save = 1;
	}else{
			if(val == 1)
			{
				ts->is_calling_save = 1;
			...
			{
				VIVO_TS_LOG_ERR("%s: Invalide number of parameters passed(%d)\n", __func__,val);
				return -EINVAL;
			}
	}
					...
					struct kobj_attribute *attr,  const char *buf, size_t count)
{
	...
	VIVO_TS_LOG_INF("%s: edge_suppress_switch is  = %d, doze_status = %d\n", __func__,edge_suppress_switch, doze_status);
...
static int gtp_enter_edge_restrain(struct goodix_ts_data *ts)
{
	...
	if(sleep_flag == 1)
	{
		VIVO_TS_LOG_INF("because touchscreen is suspend mode, so don't write register\n");
		return 0;
	}
...
/*[BUGFIX]PR-667466. TP INT pull-up enable.*/
    ...
    {
        VIVO_TS_LOG_INF("create proc entry %s success", GT91XX_CONFIG_PROC_FILE);
    }
...
//#endif
	...
	if (ret != 0) {
		VIVO_TS_LOG_INF("GTP Product id doesn't match. ret=%d\n", ret);
		//goto exit_free_irq;
	}
...
static int goodix_ts_suspend(struct device *dev)
{
	...
	VIVO_TS_LOG_INF("%s enter, sleep_flag=%d\n", __func__, sleep_flag);
...
static int goodix_ts_resume(struct device *dev)
{
	...
	ts->gtp_rawdiff_mode = 0;	//when goodix's apk extremely exits touchscreen will finger up
	...
	VIVO_TS_LOG_INF("~~~%s: line:%d ~~lcd_shutoff:%d~,sleep_flag=%d\n", __func__,__LINE__,ts->has_lcd_shutoff,sleep_flag);
...
static void fb_notifier_resume_work(struct work_struct *work)
{
	...
	if(ret<0) {
		VIVO_TS_LOG_INF("%s: Failed to resume device\n",__func__);	
	}
	mutex_unlock(&ts->suspend_mutex);
...
static void fb_notifier_suspend_work(struct work_struct *work)
{
	...
	if(ret) {
			VIVO_TS_LOG_INF("%s: Failed to suspend device\n",__func__);
	}
	mutex_unlock(&ts->suspend_mutex);
}
