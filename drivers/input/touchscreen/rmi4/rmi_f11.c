					...
					u8 finger_state, u8 n_finger, struct rmi_driver_data *ddata)
{
	...
	else
	{
		...
		*/
		...
		if(prev_state && ((x<106 && y>2220) || (x>1160 && y>2220)))
		{
			input_mt_slot(sensor->input, n_finger);
			input_mt_report_slot_state(sensor->input, MT_TOOL_FINGER,0);
			sensor->finger_tracker[n_finger] = 0;
		...
		else if(!prev_state && ((x<106 && y>2220) || (x>1160 && y>2220)))
		{
			sensor->finger_tracker[n_finger] = 0;
		 	return;
		}
	...
	if (finger_state != 0) {
#if 1		
		cur_report_data_sum = x + y + w_x + w_y;
		if(report_time == 0)
		{
			report_time = jiffies;
		}
		...
		{
			VIVO_TS_LOG_INF("[%s]: press more than 7s without move\n", __func__);
			w_y += 1;
		}
		report_data_sum = x + y + w_x + w_y;
#endif
#ifdef ABS_MT_PRESSURE
		input_report_abs(sensor->input, ABS_MT_PRESSURE, z);
#endif

		input_report_abs(sensor->input, ABS_MT_TOUCH_MAJOR, max(w_x, w_y));
		input_report_abs(sensor->input, ABS_MT_WIDTH_MAJOR, min(w_x, w_y));
