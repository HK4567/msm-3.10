...
extern int bl_IC_diff_gpio;////lrz add for backlight IC  R1214
...
int front_screen_flash_led_low(struct msm_led_flash_ctrl_t *fctrl)
{
	...
	FRONT_SCREEN_FLASH_DBG("%s entry vivo_screen_type %d\n", __func__, vivo_screen_type);
      ...
      if(vivo_screen_type)
      	{
     if(bl_IC_diff_gpio==1)
         mdss_set_spotlight_brightness(204);////LV52207
      else
	   mdss_set_spotlight_brightness(213);////R1214
...
int front_screen_flash_led_high(struct msm_led_flash_ctrl_t *fctrl)
{
	...
	FRONT_SCREEN_FLASH_DBG("%s entry\n", __func__);
      if(vivo_screen_type)
      	{
      	 if(bl_IC_diff_gpio==1)
            mdss_set_spotlight_brightness(239);//LV52207
            else
		mdss_set_spotlight_brightness(253);////R1214				
      	}
	return rc;
}
